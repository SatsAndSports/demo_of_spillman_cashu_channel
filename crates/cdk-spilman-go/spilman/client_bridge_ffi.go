...
CResult spilman_client_bridge_open_channel_from_token(void* ptr, const char* token, const char* charlie_pubkey, const char* alice_pubkey, uint64_t locktime, const char* keyset_info, uint64_t max_amount);
...
func clientBridgeOpenChannel(ptr unsafe.Pointer, token, charliePubkeyHex, alicePubkeyHex string, locktime uint64, keysetInfoJSON string, maxAmount uint64) (*OpenChannelResult, error) {
	...
	res := C.spilman_client_bridge_open_channel_from_token(
		ptr, cToken, cCharlie, cAlice, C.uint64_t(locktime), cKeyset, C.uint64_t(maxAmount))
